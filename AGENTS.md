...
│   ├── inventory.lua    # 背包网格 (10×8, slots/items)
│   ├── inventory_item.lua # 地面物品标记
│   └── equipment.lua    # 装备槽 (main_hand/off_hand/armor/helmet)
├── systems/
...
│   ├── inventory_system.lua # 背包系统 (拾取/装备/使用/丢弃/掉落)
│   └── inventory_ui.lua     # 背包 UI (10×8 网格渲染+交互)
├── data/
│   ├── definitions/
...
│   │   ├── item.lua    # 物品定义 (grid尺寸/类型/掉落表)
│   │   └── weapon.lua  # 武器定义库 (22 种武器)
│   └── prototypes/
│       └── entities.lua # 实体原型 (player/goblin/rat/orc/_item_placeholder)
├── utils/
...
### 运行测试
禁止自己运行 `love src`。游戏测试由用户自行执行，不要尝试启动游戏。

### 开发流程
1. 给出计划等待批准 (当前处于unreleased阶段所以可以大胆地进行计划,包括架构上的更改建议等)
2. 实现计划
3. Subagent检查未提交代码是否合理,优雅,风格统一,无逻辑问题
...
### 12. Inventory (`src/components/inventory.lua`)
```lua
{
    gridCols = 10,  -- 背包列数
    gridRows = 8,   -- 背包行数
    slots    = {},  -- slots[row][col] = itemKey
    items    = {},  -- {[itemKey] = {itemId, row, col}}
    nextKey  = 1,   -- 自增 itemKey
}
```
网格背包纯数据组件。`slots` 记录每个格子的占用（指向 itemKey），`items` 存储物品数据及位置。装备中的物品 `row=0, col=0`。

### 13. InventoryItem (`src/components/inventory_item.lua`)
```lua
{ itemId = "weapon_shortsword" }
```
地面物品实体标记组件。存在即表示该实体为可拾取的地面物品。

### 14. Equipment (`src/components/equipment.lua`)
```lua
{
    slots = {
...
```
装备槽纯数据组件。值指向 `Inventory.items` 中的 itemKey。
...
| 2 | **InventorySystem** | `inventory_system.lua` | 背包操作 (拾取/装备/使用/丢弃/掉落) |
| 3 | **AISystem** | `ai.lua` | 敌方 AI (PlayerTurnEnd → 行动 → TurnEnd) |
| 4 | **RenderSystem** | `render.lua` | 实体绘制 + 血条 + 瞄准预览 + 地面物品 |
| 5 | **WeaponSystem** | `weapon_system.lua` | Weapon 组件查询 (getBaseDamage 等) |

注册顺序 (main.lua): MapRenderer → TurnSystem → TweenSystem → InventorySystem → MovementSystem → CombatSystem → WeaponSystem → InputSystem → AISystem → RenderSystem
...
```
InventoryUI 通过 main.lua 的 `love.update` 中 `love.keyboard.isDown` 轮询 Tab/I 键开关，`love.keypressed`/`love.mousepressed` 背包开启时转发给 InventoryUI。
...
11. **中文 IME 兼容**: 中文输入法可能拦截字母键（如 `i`/`p`），导致 `love.keypressed` 和 `love.keyboard.isDown` 无法检测。背包开关（Tab）/拾取（P）在 `love.update` 中使用 `love.keyboard.isDown` 轮询，英文键盘模式下可正常工作