...
//
// 全部只经 scene 的公共查询 API（layer()/layer_count()/tile_at()）读地形，
// 不触碰 SceneImpl 内部（无需 friend）。逐层处理是因为每个 solid 层有独立的
// origin/tile 网格，不能共用一次步进再映射。
//
...
long long to_ll_sat(double v) {
    ...
    if (v > static_cast<double>(std::numeric_limits<long long>::max()))
...
// 单层上 cell(cx, cy) 是否非空（阻挡）。经公共 tile_at 查询：采样单元中心
// 以精确落回 (cx, cy)（避免边界 floor 歧义）。
bool cell_occupied(const SceneAsset& asset, int layer_index, int cx, int cy,
                   double origin_x, double origin_y, int tile_w, int tile_h) {
    const Vec2 center{
        static_cast<float>(origin_x + (static_cast<double>(cx) + 0.5) *
                                          static_cast<double>(tile_w)),
        static_cast<float>(origin_y + (static_cast<double>(cy) + 0.5) *
                                          static_cast<double>(tile_h))};
    int v = -1;
    return tile_at(asset, layer_index, center, &v) ==
...
LayerHit segment_hit_layer(const SceneAsset& asset, int layer_index,
                           const LayerInfo& li, int tile_w, int tile_h, Vec2 a,
                           Vec2 b) {
    LayerHit hit;
    if (li.width < 1 || li.height < 1) return hit;
    const long long dim_w = li.width, dim_h = li.height;
    const double ox = static_cast<double>(li.origin_x);
    const double oy = static_cast<double>(li.origin_y);
    ...
    auto occupied_cell = [&](long long cx, long long cy, double t) -> bool {
        ...
        if (!cell_occupied(asset, layer_index, static_cast<int>(cx),
                           static_cast<int>(cy), ox, oy, tile_w, tile_h))
            return false;
        hit.exact = true;
...
TileHit segment_hits_solid(const SceneAsset& asset, Vec2 a, Vec2 b) {
    TileHit out;
    if (!finite2(a.x, a.y) || !finite2(b.x, b.y)) {
    ...
    if (tile_w <= 0 || tile_h <= 0) {
        out.result = TileQueryResult::clear;  // 无有效网格 = 无阻挡
        return out;
    }
    bool found = false;
    for (int li = 0; li < asset.layer_count(); ++li) {
        ...
        const LayerHit h =
            segment_hit_layer(asset, li, info, tile_w, tile_h, a, b);
        // 取最小 t；同 t 取层索引小者（层按索引升序扫描，严格 < 即保留先见者）。
...
AxisResult solve_axis(const SceneAsset& asset, int tile_w, int tile_h, bool is_x,
    ...
    const double tile = static_cast<double>(is_x ? tile_w : tile_h);
    const double other_tile = static_cast<double>(is_x ? tile_h : tile_w);

    for (int li = 0; li < asset.layer_count(); ++li) {
        const LayerInfo& info = asset.layer(li);
        if (!info.solid) continue;
        const double ox = static_cast<double>(info.origin_x);
        const double oy = static_cast<double>(info.origin_y);
        const double origin = is_x ? ox : oy;
        const long long dim = is_x ? info.width : info.height;
        const double cross_origin = is_x ? oy : ox;
        const long long cross_dim = is_x ? info.height : info.width;
        ...
        auto occupied_at = [&](long long c, long long j) -> bool {
            ...
            return cell_occupied(asset, li, cx, cy, ox, oy, tile_w, tile_h);
        };
...
double nudge_clear(const SceneAsset& asset, double pos, bool positive_dir,
                   double other_pos, double w, double h, bool is_x) {
    const double inf = std::numeric_limits<double>::infinity();
    for (int i = 0; i < kNudgeMax; ++i) {
        const Rect r = is_x ? Rect{static_cast<float>(pos),
                            ...
                            : Rect{static_cast<float>(other_pos),
                                   ...
                                   static_cast<float>(w), static_cast<float>(h)};
        if (rect_hits_solid(asset, r) != TileQueryResult::solid) break;
...
SweepResult sweep_move(const SceneAsset& asset, Rect box, Vec2 delta) {
    SweepResult out;
    out.box = box;
    if (!finite2(box.x, box.y) || !finite2(box.w, box.h) ||
    ...
    const int tile_h = asset.tile_height();
    if (tile_w <= 0 || tile_h <= 0) {
        out.box.x += delta.x;  // 无有效网格：整体位移、无阻挡
        out.box.y += delta.y;
        out.result = TileQueryResult::clear;
        return out;
    }

    // X 轴（cross = 原始 y 跨度），落位 + 贴边修正
    const AxisResult rx =
        solve_axis(asset, tile_w, tile_h, true, static_cast<double>(box.x),
    ...
    if (out.blocked_x) {
        box.x = static_cast<float>(nudge_clear(
            asset, static_cast<double>(box.x), delta.x > 0.0,
            static_cast<double>(box.y), box.w, box.h, true));
    }

    // Y 轴（cross = 解算后的 x 跨度），落位 + 贴边修正
    const AxisResult ry =
        solve_axis(asset, tile_w, tile_h, false, static_cast<double>(box.y),
    ...
    if (out.blocked_y) {
        box.y = static_cast<float>(nudge_clear(
            asset, static_cast<double>(box.y), delta.y > 0.0,
