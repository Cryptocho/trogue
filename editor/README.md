# trogue 编辑器使用指南（scene_exporter v3）

用 Godot 4.7 画关卡，一键导出成引擎资产（tro-scene v2 / tro-tileset v2 JSON），引擎端自动热重载。
字段级权威定义见仓库根 `AGENTS.md`「资产规范：tro-scene v2」，本文件只讲怎么操作。

## 插件入口

- 菜单：`Project > Tools > Export tro-tileset...` / `Export tro-scene...`
- headless（Agent / 批处理，在仓库根 `trogue/` 下执行）：
  ```bash
  # 首次或资源变更后先导入
  godot --headless --path editor --import
  # 导出场景（可多个，逗号分隔；连同其 TileSet 与实体贴图一起导出）
...
产物直接写进引擎的 `../assets/`（scenes / tilesets / textures），引擎运行中改动会 150ms 内热重载。
...
- 实体：Node2D 派生节点（Marker2D / Sprite2D 都行），**节点名 = 实体 id**。
...
- alternative tile 变体忽略 + warning；Sprite2D 的翻转/缩放忽略 + warning。
