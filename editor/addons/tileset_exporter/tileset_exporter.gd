...
func _find_occlusion(root: Node) -> Dictionary:
	for child in root.get_children():
		if child is Area2D:
			for subchild in child.get_children():
				if subchild is CollisionShape2D and subchild.shape is RectangleShape2D:
					var pos: Vector2 = subchild.position
					var sz: Vector2 = subchild.shape.size
					return {
						"x": pos.x - sz.x / 2.0,
						"y": pos.y - sz.y / 2.0,
						"w": sz.x,
...
func _extract_scene_tiles(ts: TileSet) -> Array:
	...
	for si in range(ts.get_source_count()):
		...
		for i in range(scene_src.get_scene_tiles_count()):
			...
			var occlusion := _find_occlusion(instance)
			instance.queue_free()
