...
// 为 Godot 纹理原点，见 scene_impl.hpp）；越界/表缺失 → 空描述
...
// SceneImpl 析构：释放图集贴图（asset 级 RAII）。
...
RenderResult draw_rect(Rect world_rect, Color color) {
    ...
    ++g_render_stats.window_checks;
    if (!IsWindowReady()) return RenderResult::WindowUnavailable;
    // ③ 绘制（color 即色块填充色；此前参数被忽略恒画白）。
    // 用浮点原语 DrawRectanglePro，不经 DrawRectangle(int) 截断——像素/网格对齐的
    // 插值位置若被截断会产生 ±1px 错位与帧间抖动。
    DrawRectanglePro(::Rectangle{world_rect.x, world_rect.y, world_rect.w,
                                 world_rect.h},
